...
func (req CreateUserReq) Valid(ctx context.Context) validator.Evaluator {
	...
	eval.CheckField(validator.MinChars(req.Bio, 10) && validator.MaxChars(req.Bio, 255), "bio", "this field must have the length between 10 and 255")

	eval.CheckField(validator.NotBlank(req.Password), "password", "password must be greather than 8 chars")
	eval.CheckField(validator.MaxChars(req.Password, 8), "password", "password must be greather than 8 chars")
